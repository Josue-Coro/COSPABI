...
CREATE TABLE cargo_extra 
    (
     id_cargo_extra INTEGER NOT NULL IDENTITY(1,1), 
...
ALTER TABLE cargo_extra ADD CONSTRAINT cargo_extra_PK PRIMARY KEY CLUSTERED (id_cargo_extra)
     WITH (
     ALLOW_PAGE_LOCKS = ON , 
     ALLOW_ROW_LOCKS = ON )
GO
...
CREATE TABLE pago 
    (
     ...
     codigo_recaudacion VARCHAR (50) , 
     url_pasarela VARCHAR (500) , 
     qr_url VARCHAR (500) , 
...
ALTER TABLE pago 
    ADD CONSTRAINT pago_caja_FK FOREIGN KEY 
    ( 
     caja_id_caja
    ) 
    REFERENCES caja 
