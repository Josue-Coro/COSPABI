...
namespace CapaDato
{
    ...
    {
        public CM_Medidor.CM_MedidorListado Listar(
            string busqueda, int pagina, int tamanoPagina)
        {
            var resultado = new CM_Medidor.CM_MedidorListado { Medidores = new List<CM_Medidor>() };
            try
            {
                    ...
                    using (SqlCommand cmd = new SqlCommand("sp_listar_medidores", conexion))
                    {
                        ...
                        using (SqlDataReader dr = cmd.ExecuteReader())
                        {
                            ...
                            if (dr.NextResult())
                            {
                                while (dr.Read())
                                {
                                    resultado.Medidores.Add(new CM_Medidor()
                                    {
        ...
        public CM_Medidor.CM_MedidorListado ListarRegistrar(
            string busqueda, int pagina, int tamanoPagina)
        {
            var resultado = new CM_Medidor.CM_MedidorListado { Medidores = new List<CM_Medidor>() };
            try
            {
                using (SqlConnection conexion = new SqlConnection(CD_Conexion.cn))
                {
                    using (SqlCommand cmd = new SqlCommand("sp_listar_medidores_socio", conexion))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.AddWithValue("@Busqueda", busqueda ?? "");
                        cmd.Parameters.AddWithValue("@Pagina", pagina);
