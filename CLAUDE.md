...
- Controller actions return `JsonResult` shaped `{ exito, mensaje, ...data }`; views consume via jQuery AJAX. Page actions return `View()`.
...
- Tailwind CDN doesn't generate decimal sizes like `w-4.5`; use inline `style="width:18px"` for icon sizing.
